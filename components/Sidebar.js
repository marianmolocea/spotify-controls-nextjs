...
function Sidebar() {
  ...
  useEffect(() => {
    if (sportifyApi.getAccessToken()) {
      sportifyApi.getUserPlaylists().then((data) => {
  ...
  return (
    <div
      className="text-gray-500 p-5 text-xs border-r border-gray-900 overflow-y-scroll 
    ...
    >
      <div className="space-y-4">
        ...
        {playlists.map(({ id, name }) => (
          <p key={id} onClick={() => setPlaylistId(id)} className="cursor-pointer hover:text-white">
            {name}.
          </p>
        ))}
      </div>
